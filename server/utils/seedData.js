...
const projectsData = [
  {
    ...
    challenges: [
      "Implementing efficient ADO.NET data access without ORM overhead",
      "Managing session state and security permissions for admin actions"
    ],
    futureImprovements: [
    ...
    architecture: "Three-tier Architecture (Presentation, Business Logic, Data Access via ADO.NET)",
    databaseDesign: "Normalized SQL Server DB with Employee, Department, Role, Audit tables."
  },
  {
...
const seedDB = async () => {
  try {
    ...
    console.log('[Seed] Database seeded successfully with initial Projects!');
    process.exit(0);
  } catch (error) {
